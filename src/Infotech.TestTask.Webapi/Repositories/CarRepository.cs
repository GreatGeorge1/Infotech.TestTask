...
namespace Infotech.TestTask.Webapi.Repositories
{
    public class CarRepository : ICarRepository
    {
        ...
        public async Task<IReadOnlyList<Car>> GetByOwner(long personId)
        {
        ...
        public async Task<IReadOnlyList<Car>> GetByOwner(string personExternalId)
        {
            var person = await _context.People
                .AsNoTracking()
                .Include(p => p.PersonCars)
        ...
        public async Task<IReadOnlyList<Person>> GetOwners(long carId)
        {
        ...
        public async Task<IReadOnlyList<Person>> GetOwners(string carNationalId)
        {
            var car = await _context.Cars.AsNoTracking()
                .Include(car => car.PersonCars)
